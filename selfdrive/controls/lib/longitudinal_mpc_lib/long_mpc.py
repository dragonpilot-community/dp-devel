...
def gen_long_model():
  ...
  model.p = vertcat(a_min, a_max, x_obstacle, prev_a, desired_TF, lead_t_follow, lead_danger_factor)
...
def gen_long_ocp():
  ...
  lead_t_follow = ocp.model.p[5]
  lead_danger_factor = ocp.model.p[6]

  ocp.cost.yref = np.zeros((COST_DIM, ))
  ocp.cost.yref_e = np.zeros((COST_E_DIM, ))

  desired_dist_comfort = get_safe_obstacle_distance(v_ego, desired_TF, lead_t_follow)
...
class LongitudinalMpc:
  ...
  def update_TF(self, carstate):
    if carstate.distanceLines == 1: # Traffic
      ...
      x_vel = [0.0,  1,    2.78,  5.56,   8.33,  11.11, 13.89, 16.67, 19.44, 22.22, 25.0, 27.78, 30.56, 33.33, 36.11, 38.89, 41.67]
      y_dist = [1.2, 1.33, 1.376, 1.4,    1.413, 1.418, 1.44,  1.48,  1.5,   1.5,   1.5,  1.53,  1.56,  1.6,  1.64,  1.68,  1.72]
      self.desired_TF = np.interp(carstate.vEgo, x_vel, y_dist)
    else:
