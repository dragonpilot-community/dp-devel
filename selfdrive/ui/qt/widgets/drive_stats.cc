...
DriveStats::DriveStats(QWidget* parent) : QFrame(parent) {
  ...
  auto add_stats_layouts = [=](const QString &title, StatsLabels& labels) {
    ...
    grid_layout->addWidget(newLabel(DriveStats::tr("Drives"), "unit"), row + 1, 0, Qt::AlignLeft);
    grid_layout->addWidget(labels.distance_unit = newLabel(getDistanceUnit(), "unit"), row + 1, 1, Qt::AlignLeft);
    grid_layout->addWidget(newLabel(DriveStats::tr("Hours "), "unit"), row + 1, 2, Qt::AlignLeft);

    main_layout->addLayout(grid_layout);
  };

  add_stats_layouts(DriveStats::tr("ALL TIME"), all_);
  main_layout->addStretch();
  add_stats_layouts(DriveStats::tr("PAST WEEK"), week_);
