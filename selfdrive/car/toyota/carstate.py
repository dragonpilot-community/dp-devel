...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self._reverse_acc_change = Params().get_bool('dp_toyota_reverse_acc')
  ...
  def update(self, cp, cp_cam):
    ...
    if self.dp_toyota_lkas_btn_link:
      if not self.read_lkas_btn_init or self.read_lkas_btn != cp_cam.vl["LKAS_HUD"]['LKAS_STATUS']:
        self.read_lkas_btn_init = True
        self.read_lkas_btn = cp_cam.vl["LKAS_HUD"]['LKAS_STATUS']
        put_nonblocking('dp_lateral_lanelines', str(int(max(self.read_lkas_btn, 0)))) # LKAS_HUD just light indicator so 1 or 0
        put_nonblocking('dp_last_modified',str(floor(time.time())))
    ...
    #if self.CP.carFingerprint in (TSS2_CAR - RADAR_ACC_CAR):
    #  self.lkas = cp_cam.vl["LKAS_HUD"]['LKAS_STATUS']
